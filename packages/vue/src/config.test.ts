...
import { createWorkflowFromConfig } from "./config.js";
import { createGlowTour } from "./glow-tour.js";

describe("vue config entry point", () => {
  test("createWorkflowFromConfig needs no generic or cast to run through createGlowTour", async () => {
    ...
    // to what `createGlowTour().start(...)` expects, with no `<VNodeChild>` type argument and no
