...
- SSR references: `apps/ssr-react` (Next.js), `apps/ssr-vue` (Nuxt), `apps/ssr-solid` (SolidStart), `apps/ssr-angular` (Angular SSR).
...
5. If the change touches or could affect SSR/hydration/browser-global access, build the packages, then run all four SSR suites:
...
A change is complete only when its relevant tests have fresh passing output, required package behavior has been checked in the browser through `apps/playground`, SSR changes pass all four SSR apps, and bundle-size constraints remain justified. If any required validation cannot be run, state exactly what remains unverified.
